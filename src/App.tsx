...
export default function App() {
  ...
  const handleClarice = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.output) setAnswerField(json.output);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  /* ---------- Seleção de texto  ---------- */
  const handleSelection = (e: React.SyntheticEvent<HTMLDivElement>) => {
...
// === Config ===
const ENDPOINT = 'http://10.195.180.105:5678/webhook/frida-risk-question';
  ...
  history: [
    {
      ...
      occurenceDescription: "A Sra. Usuária 1 compareceu ao atendimento agendado com a especialista e inserida em acompanhamento por meio do PAEFI. A família estava abrigada e encaminhada ao CREAS para inserção em acompanhamento. A Sra. Usuária 1 foi vítima de violência doméstica e está sob medida protetiva. O Programa de Segurança Preventiva Viva Flor foi acionado para garantir atendimento prioritário da Polícia Militar. Foi repassada a 5ª autorização do auxílio vulnerabilidade e solicitada a 6ª parcela do excepcional, a 1ª do vulnerabilidade, inserção no programa Prato Cheio e solicitação da cesta básica."
    },
    {
      occurrenceDate: "27/02/2025",
      occurenceDescription: "A família está em situação de vulnerabilidade socioeconômica. A responsável familiar é vítima de violência doméstica. A família é composta pela genitora, 02 adolescentes e 02 crianças (16, 15, 10 e 05 anos). A filha adolescente de 16 anos está grávida. A responsável familiar e os filhos estavam abrigados e foram encaminhados ao CREAS para acompanhamento."
    },
    {
      occurrenceDate: "27/01/2025",
      occurenceDescription: "Em atenção ao Memorando GEACAF, foi oficiada a Diretoria de Benefícios (DIBES) via SEI para priorização na listagem de requisição para benefícios excepcionais. A família encontra-se acolhida desde 19/09/2024 e é natural de Brasília/DF. A Sra. Usuária 1 recebeu um cargo em comissão na Unidade de Engenharia e Arquitetura da Secretaria de Estado de Justiça e Cidadania do DF. Iniciou acompanhamento em saúde mental no CAPS II. Seu filho foi matriculado e frequenta o 3º ano da Escola Classe e iniciou acompanhamento no CAPSi. Sua filha cursa o 1º ano no Centro de Educação Infantil 08 e também iniciou acompanhamento no CAPSi."
    },
    {
      occurrenceDate: "27/12/2024",
      occurenceDescription: "Em articulação com a SEDES, a acolhida está conquistando autonomia com uma atividade remunerada. Está em acompanhamento de saúde mental no CAPS. Foi contemplada com um cargo em comissão na Unidade de Engenharia e Arquitetura da Secretaria de Estado de Justiça e Cidadania do DF. Seu filho foi matriculado e frequenta o 3º ano da Escola Classe, e iniciou acompanhamento no CAPSi. Sua filha cursa o 1º ano no Centro de Educação Infantil 08 e também já iniciou acompanhamento em saúde mental. O marido dela não deixa ela usar a roupa que ele quer. Ele escolhe o que ela veste. Se ela recusa, ele fica extremamente irritado."
    },
    {
      occurrenceDate: "21/09/2024",
      occurenceDescription: "Encaminhamos USUÁRIA 1 para continuidade no acompanhamento familiar devido à vulnerabilidade e risco pessoal/social. Esteve acolhida na Casa Abrigo entre 08/08/2024 e 19/09/2024 devido à violência doméstica relatada. No desligamento, solicitou o Dispositivo de Proteção Preventiva na SSP. Foi orientada a buscar suporte para viabilização de direitos e superação das situações de vulnerabilidade e risco pessoal/social."
    }
  ]
} as const ;
...
export default function App() {
  ...
  const callApi = async () => {
    ...
    try {
      ...
      setOutcome((Object.keys(flags) as (keyof RespostaFlags)[]).find((k) => flags[k]) ?? null);

      const compInfo = (ai.complementaryInformation || []).map((c: string) => `• ${c}`).join('\n');
      setAnswerField(`${ai.trechoJustificativo}\n\n${compInfo}`);
      setObservacao(ai.observacaoComplementar || '');
    } catch (e) {
      console.error(e);
...
const renderButton = (label: string) => {
  ...
  const getDialogContent = () => {
    ...
    return (
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers sx={{ backgroundColor: '#121212', color: '#fff' }}>
          <Typography variant="h6" gutterBottom>Informações Complementares</Typography>
          <ul style={{ paddingLeft: '1.5rem' }}>
            {infoList.map((item: string, index: number) => (
              <li key={index}>{item}</li>
            ))}
          ...
          <Typography variant="h6" sx={{ mt: 3 }} gutterBottom>Histórico de Ocorrências</Typography>
          {historyList.map((entry: any, index: number) => (
            <Box key={index} sx={{ mb: 2, p: 2, border: '1px solid #444', borderRadius: 2, backgroundColor: '#1e1e1e' }}>
              <Typography variant="subtitle2" sx={{ color: '#aaa' }}>
                {entry.occurrenceDate}
              </Typography>
              <Typography variant="body2" sx={{ mt: 1 }}>
  ...
  // === JSX ===
  return (
    <Box sx={{ p: 3, display: 'flex', flexDirection: 'column', gap: 2, maxWidth: 900, mx: 'auto' }}>
      <Select fullWidth displayEmpty value={selectedQuestion} onChange={handleSelect} renderValue={(v) => v || 'Selecione uma pergunta…'}>
        {questions.map((q) => (
          <MenuItem key={q} value={q} sx={{ whiteSpace: 'normal', lineHeight: 1.3 }}>
            {q}
          </MenuItem>
        ))}
      </Select>

      <Box sx={{ display: 'flex', gap: 2 }}>{['SIM', 'NÃO', 'NS', 'NA'].map(renderButton)}</Box>

      <Box sx={{ display: 'flex', gap: 2 }}>
        <TextField fullWidth multiline minRows={6} maxRows={12} label="Resposta" value={answerField} onChange={(e) => setAnswerField(e.target.value)} placeholder="Trecho justificativo + informações complementares" />
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <IconButton color="primary" onClick={callApi} disabled={!selectedQuestion} sx={{ border: 1, borderColor: 'primary.main' }}>
            <PlayArrowIcon />
          </IconButton>
          <IconButton color="secondary" onClick={openWindowWithApiData} disabled={!lastRawJson} sx={{ border: 1, borderColor: 'primary.main' }}>
            <OpenInNewIcon />
          </IconButton>
        </Box>
      </Box>

      <TextField fullWidth multiline minRows={6} maxRows={12} label="Dados da API (observação complementar)" value={observacao} InputProps={{ readOnly: true }} />

      <Typography variant="caption" sx={{ color: 'text.secondary' }}>
        • Prova de conceito - Preenchimento do formulário Frida, com assistência de Inteligência Artificial •
      </Typography>
      {getDialogContent()}
      ...
      >
        <CircularProgress color="inherit" />
        <Typography variant="subtitle1" sx={{ mt: 2 }}>
          Aguarde... IA processando fazendo análise dos dados sociais, aguarde...
        </Typography>
      </Backdrop>
    </Box>
  );
